...
int[] arraySum(int[] array) // ДОП ДЗ СДЕЛАТЬ РЕФАКТОРИНГ - вместо двух циклов, сделать один цикл, так как условия почти одинаковые!!!
{
    ...
    for (int i = 0; i < array.Length / 2; i++)
    {
        ...
        {
            newArray[i] = array[i] * array[(array.Length - 1 - i)/2];//перемножаем первый и посл эл-ты
        }
        else //если нечетное кол-во эл-ов в массиве
        
        {
            newArray[i] = array[i] * array[array.Length - 1 - i];
            newArray[newArray.Length - 1] = array[array.Length / 2 + 1]; //добавляем этот средний эл-т в конец
        }
    }
    return newArray;
}
